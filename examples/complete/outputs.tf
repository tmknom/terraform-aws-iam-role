# Output Configuration
#
# https://www.terrafsorm.io/docs/configuration/outputs.html

output "iam_role_arn" {
  value = "${module.iam_role.iam_role_arn}"
}

output "iam_role_create_date" {
  value = "${module.iam_role.iam_role_create_date}"
}

output "iam_role_unique_id" {
  value = "${module.iam_role.iam_role_unique_id}"
}

output "iam_role_description" {
  value = "${module.iam_role.iam_role_description}"
}

output "iam_role_name" {
  value = "${module.iam_role.iam_role_name}"
}

output "iam_policy_id" {
  value = "${module.iam_role.iam_policy_id}"
}

output "iam_policy_arn" {
  value = "${module.iam_role.iam_policy_arn}"
}

output "iam_policy_description" {
  value = "${module.iam_role.iam_policy_description}"
}

output "iam_policy_name" {
  value = "${module.iam_role.iam_policy_name}"
}

output "iam_policy_path" {
  value = "${module.iam_role.iam_policy_path}"
}

output "iam_policy_document" {
  value = "${module.iam_role.iam_policy_document}"
}
